...
TOKEN = "7409106709:AAEVVWWLArx1gMNyiR4ZsoiM77LPYsxycNo"
CHAT_ID = "5395282683"
...
def process_results(results, is_video=False):
    ...
    if is_video:
        ...
        writer = cv2.VideoWriter(annotated_path, cv2.VideoWriter_fourcc(*'mp4v'), 30, (width, height))
        ...
        for result in results:
            ...
            for box in result.boxes.xyxy.cpu().numpy():
                ...
                area = (x2 - x1) * (y2 - y1)
                severity = "High" if area >= MEDIUM_THRESHOLD else "Medium" if area >= LOW_THRESHOLD else "Low"
                lat, lon = generate_coordinates()
                reports.append(f"Detected {severity} pothole at ({lat}, {lon})")
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                cv2.putText(frame, severity, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            writer.write(frame)
        writer.release()
    else:
        result = results[0]
        ...
        for box in result.boxes.xyxy.cpu().numpy():
            ...
            cv2.putText(img, severity, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
